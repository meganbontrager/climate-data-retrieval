...
write.csv(locs_prism, "inputs/prism_input.csv", row.names = FALSE)
...
each_data = list.files(pattern = "PRISM_ppt", path = "raw_outputs/", full.names = TRUE)
...
all_data = lapply(each_data, read.csv, skip = 10) %>% 
...
# Do we have a complete time series? 
...
# Now save this data
write.csv(all_data, "data_tall/prism_climate_tall.csv", row.names = FALSE)
